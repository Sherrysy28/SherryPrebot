...
async def show_packages(message, cat_id):
    ...
    for i, item in enumerate(data["packages"][cat_id]):
        ...
        if stock_count <= 0:
            ...
            text += f"📦 {name} - {price:,} MMK ({stock_count})\n"

        keyboard.append([
            InlineKeyboardButton(
                name,
                callback_data=f"buy_{cat_id}_{i}"
            )
        ])
...
"""
        ...
        order_text = f"{data['categories'][cat_id]} - {pkg_name}"
...
"""
        ...
        await query.message.reply_text(
            "Reply topup screenshot with amount"
        )
...
async def addstock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            "/addstock 2_0 email@gmail.com:pass"
        )
...
"""
    ...
    if text == "🛍 Products":
        ...
        await update.message.reply_text(
            f"💰 Balance = {data['balances'][user_id]:,}"
        )
